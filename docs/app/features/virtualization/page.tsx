...
const themes: Record<Theme, BonelessTableClassNames> = {
  material: {
    ...
    toolbarSummary:
      'flex min-w-0 items-baseline gap-1.5 text-sm text-slate-500 [&_strong]:font-bold [&_strong]:text-slate-900',
    toolbarActions:
    ...
    table: 'min-w-[780px]',
    headerGroup: 'bg-slate-50/95 backdrop-blur',
    headerRow: 'border-b border-slate-200',
    header:
      'group flex min-w-0 items-center gap-1 overflow-hidden px-4 py-3 data-[align=right]:justify-end',
    headerButton:
      'flex min-w-0 flex-1 items-center gap-1 overflow-hidden border-0 bg-transparent p-0 text-left text-xs font-bold tracking-wide text-slate-500 uppercase data-[align=right]:flex-row-reverse data-[align=right]:text-right',
    headerLabel: 'min-w-0 overflow-hidden text-ellipsis whitespace-nowrap',
    sortIndicator:
      'ml-auto shrink-0 text-indigo-500 opacity-0 transition-opacity group-hover:opacity-100 group-focus-within:opacity-100 data-[reveal=always]:opacity-100 data-[align=right]:mr-1 data-[align=right]:ml-0',
    filters: 'flex flex-wrap gap-3 border-b border-slate-200 bg-indigo-50/50 px-5 py-3',
    filterItem: 'grid min-w-40 flex-1 gap-1 sm:max-w-56',
    filterLabel: 'text-[11px] font-bold tracking-wide text-slate-500 uppercase',
    ...
    filter:
      'h-9 min-w-0 rounded-xl border border-slate-200 bg-white px-3 text-sm shadow-sm outline-none focus-within:border-indigo-500 focus-within:ring-2 focus-within:ring-indigo-100 focus:border-indigo-500 focus:ring-2 focus:ring-indigo-100 data-[slot=filter-text]:flex data-[slot=filter-text]:items-center data-[slot=filter-text]:gap-2 [&_[data-slot=filter-input]]:w-full [&_[data-slot=filter-input]]:min-w-0 [&_[data-slot=filter-input]]:border-0 [&_[data-slot=filter-input]]:outline-none',
    columnMenu:
  ...
  brutalist: {
    ...
    toolbarSummary:
      'flex min-w-0 items-baseline gap-1.5 font-mono text-sm text-slate-950 [&_strong]:text-lg',
    toolbarActions:
    ...
    scroller: 'overflow-auto',
    table: 'min-w-[780px]',
    headerGroup: 'bg-[#95e1d3]',
    headerRow: 'border-b-3 border-slate-950',
    header:
      'group flex min-w-0 items-center gap-1 overflow-hidden border-r-2 border-slate-950 px-4 py-3 last:border-r-0 data-[align=right]:justify-end',
    headerButton:
      'flex min-w-0 flex-1 items-center gap-1 overflow-hidden border-0 bg-transparent p-0 text-left font-mono text-xs font-black tracking-wide uppercase text-slate-950 data-[align=right]:flex-row-reverse data-[align=right]:text-right',
    headerLabel: 'min-w-0 overflow-hidden text-ellipsis whitespace-nowrap',
    sortIndicator:
      'ml-auto shrink-0 opacity-0 group-hover:opacity-100 group-focus-within:opacity-100 data-[reveal=always]:opacity-100 data-[align=right]:mr-1 data-[align=right]:ml-0',
    filters: 'flex flex-wrap gap-3 border-b-3 border-slate-950 bg-[#d6f5ff] px-5 py-3',
    filterItem: 'grid min-w-40 flex-1 gap-1 sm:max-w-56',
    filterLabel: 'font-mono text-[11px] font-black uppercase',
    ...
    filter:
      'h-9 min-w-0 border-2 border-slate-950 bg-white px-2 font-mono text-sm outline-none focus-within:bg-[#ffdf3f] focus:bg-[#ffdf3f] data-[slot=filter-text]:flex data-[slot=filter-text]:items-center data-[slot=filter-text]:gap-2 [&_[data-slot=filter-input]]:w-full [&_[data-slot=filter-input]]:min-w-0 [&_[data-slot=filter-input]]:border-0 [&_[data-slot=filter-input]]:font-mono [&_[data-slot=filter-input]]:outline-none',
    columnMenu:
      ...
      },
      {
        ...
        meta: {
          bonelessTable: {
            ...
            filtering: { type: 'select', options: owners },
          },
        },
      },
      {
        ...
        meta: {
          bonelessTable: {
            ...
            filtering: { type: 'select', options: statuses },
          },
        },
      },
      {
