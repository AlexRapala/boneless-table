...
In Vercel Project Settings → Build and Deployment, set **Root Directory** to `.` (the repository root), not `docs`. The root `vercel.json` then installs workspace dependencies, builds the docs app with `npm run build:docs`, and deploys `docs/.next`.
